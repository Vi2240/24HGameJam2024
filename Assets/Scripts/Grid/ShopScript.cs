...
public class ShopScript : MonoBehaviour
{
    ...
    public void BuyTurret()
    {
        ...
        {
            BasicGrid[] myItems = FindObjectsOfType(typeof(BasicGrid)) as BasicGrid[];

            foreach (BasicGrid item in myItems)
            {
    ...
    {
        Vector3 mousePos = UnityEngine.Camera.main.ViewportToWorldPoint(Input.mousePosition);
        turretUpgradeVisuals.transform.position = UnityEngine.Camera.main.WorldToViewportPoint(mousePos);
    ...
    public void UpgradeTurretButton()
    {
        ...
        objectToUpgrade.gameObject.transform.GetComponent<BasicGrid>().upgradeBought = true;
        if (resourceVault.items[0] >= 50f)
        {
